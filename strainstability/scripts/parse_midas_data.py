...
data_cohort = state_utils.get_cohort()
...
def calculate_relative_depth_threshold_map(sample_coverage_histograms, samples, min_nonzero_median_coverage=5, lower_factor=0.3, upper_factor=3):
...
def pipe_snps(species_name, min_nonzero_median_coverage=config.pipe_snps_min_nonzero_median_coverage, lower_factor=config.pipe_snps_lower_depth_factor, upper_factor=config.pipe_snps_upper_depth_factor, min_samples=config.pipe_snps_min_samples, debug=False):
    ...
    from config import postproc_idx_dic
...
#     If there are 4 samples, then they must be spread across at least 2 people. 
    ...
    ref_freq_file = bz2.BZ2File(postproc_idx_dic["ref_freq_file"] % (data_directory, species_name),"r")
    ...
    while True:
        ...
        if config.MIDAS_ver == "1.3.2":
            gene_name = info_items[-4]
        else:
            gene_name = info_items[6]
            
        if config.MIDAS_ver == "1.3.2":
            site_id_items = info_items[0]
            contig = info_items[1]
            location = site_id_items
        
        elif config.MIDAS_ver == "mod":
...
def parse_snps(species_name, debug=False, allowed_samples=[], allowed_genes=[], allowed_variant_types=['1D','2D','3D','4D'], initial_line_number=0, chunk_size=1000000000):
    ...
    items = line.split()[1:]    
    #samples = numpy.array(allowed_samples)
    samples = numpy.array(items)
    #samples = numpy.array(sample_utils.parse_merged_sample_names(items))
    ...
    #    samples = list(allowed_sample_set)
