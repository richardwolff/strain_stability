...
def create_hash():
    return ''.join(rand.choice(string.ascii_uppercase + string.digits) for _ in range(5))
...
def calculate_secondary_distance_matrix(target_avg_fs, target_avg_Ds, cluster_As, cluster_Ds):
    ...
    distance_matrix_1 = []
    distance_matrix_2 = []
    for i in range(0,target_avg_fs.shape[0]):
...
def calculate_distance_matrix(cluster_As, cluster_Ds):
    ...
    for i in range(0,cluster_fs.shape[0]):
...
def cluster_snps(cluster_As, cluster_Ds, min_coverage = config.cluster_min_coverage, max_num_snps_to_cluster=1000):
    ...
    if cluster_As.shape[0]>max_num_snps_to_cluster or cluster_As.shape[0]<2.5:
        ...
        if Smax <= .3:
            ...
            nodes_hash = {a:"LOW QUAL" for a in numpy.unique(nodes)}
            
            nodes = numpy.array([nodes_hash[node] for node in nodes])
            
        else:
            
            nodes = fcluster(Z, num_clusters, criterion="maxclust")
            nodes_hash = {a:create_hash() for a in numpy.unique(nodes)}
...
def cluster_secondary_snps_by_distance(cluster_map, cluster_As, cluster_Ds, max_num_snps_to_cluster=2000, max_d=config.cluster_distance_threshold_reads,min_coverage=10,min_d=0):
...
#     # Now do leftovers
    #leftover_cluster_map = fast_cluster_snps_by_distance(leftover_cluster_As, leftover_cluster_Ds,max_d=max_d,min_d=min_d,min_coverage=min_coverage)
    leftover_cluster_map = cluster_snps(leftover_cluster_As, leftover_cluster_Ds)
...
def fast_cluster_snps_by_distance(cluster_As, cluster_Ds, max_num_snps_to_cluster=1000, max_d=config.cluster_distance_threshold_reads,min_coverage=10,min_d=0):
    ...
    if num_snvs < max_num_snps_to_cluster:
        return cluster_snps(cluster_As, cluster_Ds)
    ...
    for snp_idx in range(0,num_snvs):
    ...
    seed_cluster_map = cluster_snps(seed_cluster_As, seed_cluster_Ds)
    rest_cluster_map = cluster_secondary_snps_by_distance(seed_cluster_map, rest_cluster_As, rest_cluster_Ds, max_d=max_d,min_d=min_d)
    new_cluster_map = {}
